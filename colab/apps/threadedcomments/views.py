...
def comment_form(request, content_type=None, object_id=None, edit_id=None, parent_id=None, model=ThreadedComment, template_name='threadedcomments/comment_form.html', context_processors=[], extra_context={}):
    """
    Get form with WMD Editor for comments.
    ...
    if edit_id:
        instance = get_object_or_404(model, id=edit_id)
        editing = True
        comment_url = reverse('tc_comment_edit', kwargs={'edit_id': edit_id})
        
        content_type = ContentType.objects.get_for_model(ThreadedComment).id
        object_id = edit_id
    else:
        instance = None
        editing = False
        ...
        if parent_id:
            ...
            comment_url = reverse('tc_comment_parent', kwargs=kwargs)
            
            content_type = ContentType.objects.get_for_model(ThreadedComment).id
            object_id = parent_id
        else:
            comment_url = reverse('tc_comment', kwargs=kwargs)
    
    form_id = '%s-%s' % (str(content_type), str(object_id))
    comment_form = WmdCommentForm(instance=instance)
